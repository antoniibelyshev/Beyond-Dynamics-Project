...
def choose_trajectories_low_and_high_energies(N_traj_le=200, N_traj_he=400, le_boundary=-2, he_boundary=-1, path="../data"):
    double_pendulum_data = np.load(f"{path}/double_pendulum_2000_500_x1.5pi_v0.5.npz")
    ...
    if (energies < le_boundary).sum() < N_traj_le:
        ...
        np.savez(f"{path}/double_pendulum_low_energy.npz", data=data_le, params=params_le)
    ...
    if (energies > he_boundary).sum() < N_traj_he:
        ...
        np.savez(f"{path}/double_pendulum_high_energy.npz", data=data_he, params=params_he)
