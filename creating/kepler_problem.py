...
def create_trajectories(N_traj=200, traj_len=500, save=True, energy_interval=(-0.5, -0.15), momentum_interval=(0, 1), orientation_interval=(0, pi / 4), path="../data"):
    """
    ...
    if save:
        np.savez(f"{path}/kepler_problem.npz", data=data, params=np.stack((energies, angular_momentums, thetas0), axis=1))
